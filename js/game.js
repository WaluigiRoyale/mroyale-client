...
function genAddSkinButton(screen) {
    for (var i=0; i<SKINCOUNT; i++) {
        ...
        elem.style["background-image"] = "url('" + ASSETS_URL + "img/game/smb_skin" + i +".png')";
...
GameState.prototype.load = function(data) {
    ...
    $.ajax({
        'url': "game/" + data.game + ".json?v=" + VERSION,    //get level data
        'type': "GET",
        'timeout': 0x1388,
        'success': function(data) {
            app.load(data);
...
PlayerObject.ANIMATION_RATE = 0x2;
...
Display.prototype.drawUI = function() {
    ...
    if (0x3 >= this.game.victory && 0 !== this.game.victory) {
        ...
        txt = 'x' + (0x9 >= this.game.coins ? '0' + this.game.coins : this.game.coins); // + this.game.coins : this.game.coins
